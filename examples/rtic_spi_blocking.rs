    ...
    fn idle(cx: idle::Context) -> ! {
        ...
        loop {
...

            {
                ...
                // burst of data on your scope. Since the peripheral
                // needs to wait for receive data, you'll notice a
                // larger gap between transfers. Use your scope to
                // compare this gap with the gap shown in the pipelined
                // writes.
                let mut buffer = BUFFER;
                for idx in 0u32..16 {
                    ...
                    let expected = buffer;
                    spi.transfer(&mut buffer).unwrap();
                    defmt::assert_eq!(buffer, expected);
                }
                ...
                // on your scope. The gap between adjacent writes is
                // smaller than the gap between adjacent transfers,
                // since writes can overlap.
                let mut buffer = BUFFER;
                for idx in 0..16 {
                    buffer.fill(SENTINEL.rotate_right(idx));
                    spi.write(&buffer).unwrap();
                ...
                // Writes with intentional flushes. This prevents
                // overlapping writes, and you should observe a
                // gap between adjacent writes that's equivalent
                // to the transfer behaviors.
                let mut buffer = BUFFER;
                for idx in 0..16 {
                    buffer.fill(SENTINEL.rotate_right(idx));
                    spi.write(&buffer).unwrap();
