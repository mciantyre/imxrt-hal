...
//! let mut a1 = adc::AnalogInput::new::<_, 1>(pads.gpio_ad_b1.p02);
//!
...
impl AnalogInput {
    ...
    /// ADC input. You're responsible for ensuring that the pin is
    /// compatible with the ADC instance you'll use for reading.
    pub fn new<P, const N: u8>(mut pin: P) -> Self
    where
        P: Pin<N>,
    {
        prepare(&mut pin);
        Self { channel: P::INPUT }
    }
...
impl DmaSource {
    ...
    pub(crate) fn instance(&self) -> u8 {
        ral::adc::number(&*self.adc.reg).unwrap()
    }
}
